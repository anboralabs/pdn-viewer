...
import co.anbora.labs.pdn.search.ImageTagManager;
...
import co.anbora.labs.pdn.thumbnail.actions.ToggleTagsPanelAction;
import co.anbora.labs.pdn.ui.ImageComponent;
import co.anbora.labs.pdn.ui.ImageComponentDecorator;
import co.anbora.labs.pdn.ui.ThumbnailComponent;
import co.anbora.labs.pdn.ui.ThumbnailComponentUI;
...
final class ThumbnailViewUI extends JPanel implements DataProvider, Disposable {
  ...
  public static Navigatable[] EMPTY_NAVIGATABLE_ARRAY = new Navigatable[0];
  ...
  private void createUI() {
    if (cellRenderer == null || list == null) {
      ...
      imageComponent.setTransparencyChessboardVisible(
          chessboardOptions.isShowDefault());
      imageComponent.setTransparencyChessboardCellSize(
          chessboardOptions.getCellSize());
      imageComponent.setTransparencyChessboardWhiteColor(
          chessboardOptions.getWhiteColor());
      imageComponent.setTransparencyChessboardBlankColor(
          chessboardOptions.getBlackColor());
      ...
      ThumbnailComponentUI componentUI =
          (ThumbnailComponentUI)ThumbnailComponentUI.createUI(cellRenderer);
      ...
      ActionGroup actionGroup = (ActionGroup)actionManager.getAction(
          ThumbnailViewActions.GROUP_TOOLBAR);
  ...
  private JPanel createTagPreviewPanel() {
    ...
    tagsList.setEmptyText(
        ImagesBundle.message("list.empty.text.no.tags.defined"));
    ImageTagManager imageTagManager =
        ImageTagManager.getInstance(thumbnailView.getProject());
    return ToolbarDecorator.createDecorator(tagsList)
        .setAddAction(button
                      -> JBPopupFactory.getInstance()
                             .createActionGroupPopup(
                                 IdeBundle.message("popup.title.add.tags"),
                                 new AddTagGroup(), button.getDataContext(),
        ...
        .setRemoveAction(button -> {
          ...
          if (selectedValue != null) {
            Arrays.stream(getSelection())
                .forEach(
                    virtualFile
                    -> imageTagManager.removeTag(selectedValue, virtualFile));
          }
          updateTagsPreviewModel();
        })
  ...
  private void updateTagsPreview() {
    ...
    boolean enabled = PropertiesComponent.getInstance(project).getBoolean(
        ToggleTagsPanelAction.TAGS_PANEL_VISIBLE, false);
    float splitterProportion = previewSplitter.getProportion();
    if (enabled) {
      if (splitterProportion == 1) {
        previewSplitter.setProportion(
            Float.valueOf(PropertiesComponent.getInstance(project).getValue(
                ToggleTagsPanelAction.TAGS_PANEL_PROPORTION, "0.5f")));
      }
      if (tagsPanel == null) {
      ...
      if (splitterProportion != 1) {
        PropertiesComponent.getInstance(thumbnailView.getProject())
            .setValue(ToggleTagsPanelAction.TAGS_PANEL_PROPORTION,
                      String.valueOf(splitterProportion));
      }
      previewSplitter.setProportion(1);
      previewSplitter.setSecondComponent(null);
    }
      ...
      if (root != null && root.isValid() && root.isDirectory()) {
        ...
        for (VirtualFile virtualFile : virtualFiles) {
          if (filter == null || filter.accepts(virtualFile)) {
            if (tagFilters == null ||
                Arrays.stream(tagFilters)
                    .anyMatch(tagFilter -> tagFilter.accepts(virtualFile))) {
  ...
  public void scrollToSelection() {
    ...
    if (minSelectionIndex != -1 && maxSelectionIndex != -1) {
      list.scrollRectToVisible(
          list.getCellBounds(minSelectionIndex, maxSelectionIndex));
    }
  }
  ...
  private final class ThumbnailListCellRenderer
      extends ThumbnailComponent implements ListCellRenderer {
    private final ImageFileTypeManager typeManager =
        ImageFileTypeManager.getInstance();
    ...
    public Component getListCellRendererComponent(JList list, Object value,
      ...
      if (value instanceof VirtualFile) {
        ...
        String toolTipText =
            IfsUtil.getReferencePath(thumbnailView.getProject(), file);
    ...
    public void mouseDragged(MouseEvent e) {
      ...
      if (index != -1) {
        ...
        if (!cellBounds.contains(point) &&
            (KeyEvent.CTRL_DOWN_MASK & e.getModifiersEx()) !=
    ...
    public void mouseClicked(MouseEvent e) {
      ...
      if (index != -1) {
          ...
          if (selected != null) {
            if (selected.isDirectory()) {
              ...
              FileEditorManager fileEditorManager =
                  FileEditorManager.getInstance(thumbnailView.getProject());
        ...
        if (MouseEvent.BUTTON3 == e.getButton() && e.getClickCount() == 1) {
          ...
          ActionManager actionManager = ActionManager.getInstance();
          ActionGroup actionGroup = (ActionGroup)actionManager.getAction(
              ThumbnailViewActions.GROUP_POPUP);
          ActionPopupMenu menu = actionManager.createActionPopupMenu(
              ThumbnailViewActions.ACTION_PLACE, actionGroup);
  ...
  public Object getData(@NotNull String dataId) {
    if (CommonDataKeys.PROJECT.is(dataId)) {
      ...
      return selectedFiles.length > 0
          ? PsiManager.getInstance(thumbnailView.getProject())
                .findFile(selectedFiles[0])
          : null;
    } else if (LangDataKeys.PSI_ELEMENT_ARRAY.is(dataId)) {
      return getSelectedElements();
    } else if (CommonDataKeys.NAVIGATABLE.is(dataId)) {
      VirtualFile[] selectedFiles = getSelectedFiles();
      return new ThumbnailNavigatable(
  ...
  private PsiElement[] getSelectedElements() {
    ...
    for (VirtualFile file : selectedFiles) {
      ...
      PsiElement element =
          psiFile != null ? psiFile : psiManager.findDirectory(file);
      if (element != null) {
  ...
  private final class ThumbnailNavigatable implements Navigatable {
    ...
    ThumbnailNavigatable(VirtualFile file) { this.file = file; }
        ...
        actions[i] = new AnAction(tag) {
          ...
          public void update(@NotNull AnActionEvent e) {
            e.getPresentation().setEnabledAndVisible(
                !ContainerUtil.exists(thumbnailView.getSelection(),
                                      file -> tagManager.hasTag(tag, file)));
          }
        };
      }
      actions[tagsNumber] = new AnAction(
          IdeBundle.messagePointer("action.Anonymous.text.new.tag")) {
        @Override
        public void actionPerformed(@NotNull AnActionEvent e) {
          ThumbnailView view =
              ThumbnailViewActionUtil.getVisibleThumbnailView(e);
          if (view != null) {
            VirtualFile[] selection = view.getSelection();
            if (selection.length > 0) {
              String tag = Messages.showInputDialog(
                  "", IdeBundle.message("dialog.title.new.tag.name"), null);
              if (tag != null) {
